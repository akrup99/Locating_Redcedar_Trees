# Ally Kruper
# 10/8/2024
# Doing final modeling -- see "SpeciesModeling.R" script for all of the past past attempts/data explorations
# Also see "SpeciesModeling.R" for details on source code origins
# See "SpeciesModeling_Final.R" for code that went into these (did 75/25 split instead of LOOCV and 10-fold cross-validation)

#### General Model Information ####

# these models use LOOOCV; also testing with 10-fold CV below
  # unlike models for thesis, which used 75/25 split for accuracy
  # build model on full dataset (used for predicting on new data) --> loop through with 1 observation
    # pulled as testing data until all observations have been used as testing --> take average of 
    # resulting accuracies --> those are final accuracies (For the LOOCV models; 10-fold is similar but 10 split of 90/10 involved)

### Ultimately decided for the paper to do 10-fold cross-validation!! Results were very similar to LOOCV, and 
  # 10-fold cross-validation is more commonly used. You can get usable standard deviation from is (unlike LOOCV)
...
#### LOOCV on the MS and lidar; ONF; top 3m model #####
...
# Now need to determine accuracy by looping through the model trained on each observation as testing data (each once)
  # --> take average accracy from that as final accuracy

# creating LOOCV dataset 
LOO <-  loo_cv(ModelData) # this created 92 splits --> need to cycle through each one and pull accuracy

# making empty data frame to put final accuracy data data, because that makes my brain happy
Split_Num <- rep("NA", length(LOO$splits))
Accuracy <- rep("NA", length(LOO$splits))
Pred_Other <- rep("NA", length(LOO$splits))
Pred_THPL <- rep("NA", length(LOO$splits))

LOOCV_Accuracies <- data.frame(Split_Num, Accuracy, Pred_Other, Pred_THPL)

for (i in 1:length(LOO$splits)) {
  
  # building model (uses same final variables and hyperparameters as model built on all observations)
  set.seed(42)
  Training_Data <- training(LOO$splits[[i]])
  ...
  LOOCV_Accuracies[i, "Split_Num"] <- i
  
  final_res <- final_wf %>%
    last_fit(LOO$splits[[i]])
  
  metrics <- final_res %>%
    collect_metrics() 
  
  LOOCV_Accuracies[i, "Accuracy"] <- metrics[1, ".estimate"]
  
  predictions <- final_res %>% # predictions (includes percent of trees that predicted each!)
    collect_predictions()
  
  LOOCV_Accuracies[i, "Pred_Other"] <- predictions[1, ".pred_Other"]
  LOOCV_Accuracies[i, "Pred_THPL"] <- predictions[1, ".pred_THPL"]
  
}

mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 84%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 37%

#### LOOCV on the MS and lidar; ONF; small cylinder model #####
...
#### LOOCV on the MS and lidar; OESF; top 3m model #####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 89%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 32%



#### LOOCV on the MS and lidar; OESF; small cylinder model ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 80%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 40%





#### LOOCV on the lidar only; OESF; top 3m model #####
library(tidymodels)
library(ranger)
library(vip)
library(themis)
...
mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 85%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 36%



#### LOOCV on the lidar only, OESF, small cylinder model ####
# reading in data
...
mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 82%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 39%


#### LOOCV on the lidar only, both sites, top 3m ####
# reading in data
ModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
ModelData_WH_DF <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics_DF_WH.csv")
head(ModelData) # glimpse of data
...
# creating LOOCV dataset 
LOO <-  loo_cv(ModelData) # this created splits --> need to cycle through each one and pull accuracy
...
#### LOOCV on the lidar only, both sites, small cylinder ####
# reading in data
ModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
# have to add in data for plots 11, 12, 13 (ran processing separately)
ModelData_plots111213 <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics_plots11_12_13.csv")
...
#### LOOCV on the MS and lidar, both sites, top 3m ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
mean(as.numeric(LOOCV_Accuracies$Accuracy)) # mean accuracy 81%
sd(as.numeric(LOOCV_Accuracies$Accuracy)) # sd 40%




#### LOOCV on the MS and lidar, both sites, small cylinder ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
#### Testing out 10-fold cross-validation on the lidar only; OESF; top 3m model #####
library(tidymodels)
library(ranger)
library(vip)
library(themis)
...
#### Testing out 10-fold cross-validation on the MS and lidar; ONF; top 3m model #####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
#### Testing out 10-fold cross-validation on the MS and lidar; OESF; top 3m model #####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
#### Testing out 10-fold cross-validation on the MS and lidar; OESF; small cylinder model ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
#### Testing out 10-fold cross-validation on the lidar only, OESF, small cylinder model ####
# reading in data
ModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
# have to add in data for plots 11, 12, 13 (ran processing separately)
ModelData_plots111213 <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics_plots11_12_13.csv")
...
#### Testing out 10-fold cross-validation on the lidar only, both sites, top 3m ####
# reading in data
ModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
ModelData_WH_DF <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics_DF_WH.csv")
head(ModelData) # glimpse of data
...
#### Testing out 10-fold cross-validation on the lidar only, both sites, small cylinder ####
# reading in data
ModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
# have to add in data for plots 11, 12, 13 (ran processing separately)
ModelData_plots111213 <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics_plots11_12_13.csv")
...
#### Testing out 10-fold cross-validation on the MS and lidar, both sites, top 3m ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_TreeTops_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
...
#### Testing out 10-fold cross-validation on the MS and lidar, both sites, small cylinder ####
LidarModelData <- read.csv("data/LidarMetrics/FieldTrees/Leaning_Trees_SmallCylinder_normalized_ConeClip_metrics.csv")
MSModelData <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_thesis.csv")
