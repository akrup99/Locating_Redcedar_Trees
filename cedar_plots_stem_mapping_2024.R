...
##### Re-writing plots with trees moved based on TP AND two other trees ####
# ****this seemed to fail, as they did not all typically agree, and I gave up after some time working on it in 2022***
# My suggestion is to only check the Javad-to-tree measurements if the stem map location seems significantly off when stem mapping
# 
# plot48 <- read.csv("T3_2022/Stem Maps/Plot 48_localXY.csv")
# 
# Javad_XY <- read.csv("T3_2022/Stem Maps/Plot 48_JavadOnly.csv")
# 
# TP_to_tree1 <- computeLocalTreePositions(trees = plot48[plot48$Tag_Num == 32,])
# Javad_to_tree1 <- computeLocalTreePositions(trees = Javad_data[Javad_data$Plot_Number == 48 & Javad_data$Tag_Num == 32,], azLabel = "Azimuth_to_Javad", distLabel = "Disance_to_Javad")
# tree1_X <- Javad_to_tree1[,"X"] + TP_to_tree1[,"X"] 
# 
# TP_to_tree2 <- computeLocalTreePositions(trees = plot48[plot48$Tag_Num == 34,])
# Javad_to_tree2 <- computeLocalTreePositions(trees = Javad_data[Javad_data$Plot_Number == 48 & Javad_data$Tag_Num == 34,], azLabel = "Azimuth_to_Javad", distLabel = "Disance_to_Javad")
# tree2_X <-   TP_to_tree2[,"X"] + Javad_to_tree2[,"X"]
# 
# Javad_X <- JavadXY[JavadXY$Plot_Number == 48,"X"]
# 
# tree1_Y <- TP_to_tree1[,"Y"] + Javad_to_tree1[,"Y"]
# tree2_Y <- TP_to_tree2[,"Y"] + Javad_to_tree2[,"Y"]
# 
# Javad_Y <- JavadXY[JavadXY$Plot_Number == 48,"Y"]
# 
# average_X <- (Javad_X + tree1_X + tree2_X) / 3
# average_Y <- (Javad_Y + tree1_Y + tree2_Y) / 3
# 
# Javad_X[1] - tree1_X[1]
# Javad_Y[1] - tree1_Y[1]
# tree1_Y[1] - tree2_Y[1]
...
# 
# 
#  # write.csv(x = plot48, "T3_2022/Stem Maps/average_test/Plot 48_averaged.csv")
# 
