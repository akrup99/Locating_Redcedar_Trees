...
#### Cleaning data for 502 class ####
## Need this data for class in the more immediate sense --> going to remove outliers/things that weren't successfully processed -->
  # will come back later to investigate why processing failed

all_trees <- read.csv("data/MS_data/all_trees_w_MS_values.csv")
head(all_trees)


# going to remove trees 6_37_THPL and 6_38_THPL b/c failed to get any metrics on them (think that they are outside of extent)
all_trees <- all_trees[-which(all_trees$Tree.ID == "6_37_THPL"), ]
all_trees <- all_trees[-which(all_trees$Tree.ID == "6_38_THPL"), ]

# Going to remove columns ciredrededge and cirededge because both of those failed for a couple of sets of plots/images
all_trees <- subset(all_trees, select = -c(ciredrededge, cirededge))

# removing tree 18_21_ALRU because multiple weird things seemed to happen to that tree
...
range(all_trees$msr) 
all_trees$msr == 0 ## HAS SOME TRUES --> going to drop those trees
...
# also, decided to only focus on THPL and TSHE ---> vast majority of species
barplot(table(all_trees$Species), # showing that
        ylab = "Frequency",
        xlab = "Species")
...
# writing off a .csv
# write.csv(all_trees, "data/MS_Data/all_trees_w_MS_values_cleaned_for_502.csv")


#### Cleaning data and fixing things ignored for 502 class analysis ####
# needed data asap for class --> ignored some weirdness-es --> now going back to officially fix those/clean the data for thesis use
...
#### Quick look at data; using data used for 502 class ####
all_trees <- read.csv("data/MS_Data/all_trees_w_MS_values_cleaned_for_502.csv", header = TRUE)

# specifically interested in whether the data is different between the two locations,
  # with the assumption that ONF cedars have worse health than OESF cedars
all_trees[ , 'Site'] = NA
...
cedars$Status.Code <- as.character(cedars$Status.Code)

response_vars <- all_trees[,9:20]
## some things to note:
  # there are only 42 ONF cedars; average DBH 135cm; 8 out of 42 (about 1/5) have a dead top (noted in lidar)
  # there are 114 OESF cedars; average DBH 28cm
...
# trying with nvdinir
ggplot(cedars,
       aes(x = Site, y = nvdinir)) + 
  geom_boxplot(aes(color = Site)) +
  ggtitle("Site comparison - Cedars only")

ggplot(cedars[cedars$Site == "ONF",],
       aes(x = Status.Code, y = nvdinir)) + 
  geom_boxplot(aes(color = Status.Code)) +
  ggtitle("Dead top comparison - ONF")

ggplot(all_trees,
       aes(x = Species, y = nvdinir)) + 
  geom_boxplot(aes(color = Species)) +
  ggtitle("Species comparison - both sites")
...
ggplot(all_trees,
       aes(x = Species, y = nvdinir, fill = Site)) + 
  geom_boxplot() +
  ggtitle("Species and Site")

# trying with nvdiredrededge
ggplot(cedars,
       aes(x = Site, y = nvdiredrededge)) + 
  geom_boxplot(aes(color = Site)) +
  ggtitle("Site comparison - Cedars only")

ggplot(cedars[cedars$Site == "ONF",],
       aes(x = Status.Code, y = nvdiredrededge)) + 
  geom_boxplot(aes(color = Status.Code)) +
  ggtitle("Dead top comparison - ONF")

ggplot(all_trees,
       aes(x = Species, y = nvdiredrededge)) + 
  geom_boxplot(aes(color = Species)) +
  ggtitle("Species comparison - both sites")
...
ggplot(all_trees,
       aes(x = Species, y = nvdiredrededge, fill = Site)) + 
  geom_boxplot() +
  ggtitle("Species and Site")

# trying same graphs but with cigreen

ggplot(cedars,
       aes(x = Site, y = cigreen)) + 
  geom_boxplot(aes(color = Site)) +
  ggtitle("Site comparison - Cedars only")

ggplot(cedars[cedars$Site == "ONF",],
       aes(x = Status.Code, y = cigreen)) + 
  geom_boxplot(aes(color = Status.Code)) +
  ggtitle("Dead top comparison - ONF")

ggplot(all_trees,
       aes(x = Species, y = cigreen)) + 
  geom_boxplot(aes(color = Species)) +
  ggtitle("Species comparison - both sites")
...
ggplot(all_trees,
       aes(x = Species, y = cigreen, fill = Site)) + 
  geom_boxplot() +
  ggtitle("Species and Site")
...
# trying with msrredrededge
ggplot(cedars,
       aes(x = Site, y = msrredrededge)) + 
  geom_boxplot(aes(color = Site)) +
  ggtitle("Site comparison - Cedars only")

ggplot(cedars[cedars$Site == "ONF",],
       aes(x = Status.Code, y = msrredrededge)) + 
  geom_boxplot(aes(color = Status.Code)) +
  ggtitle("Dead top comparison - ONF")

ggplot(all_trees,
       aes(x = Species, y = msrredrededge)) + 
  geom_boxplot(aes(color = Species)) +
  ggtitle("Species comparison - both sites")
...
ggplot(all_trees,
       aes(x = Species, y = msrredrededge, fill = Site)) + 
  geom_boxplot() +
  ggtitle("Species and Site")
...
# now doing mini redo of 502 class project with the shorter trees removed

library(vegan)
trees <- all_trees[,]
# creating new dataframe with only the explan variables
trees_explan <- cbind(trees[, 1:8], trees[, 21])
# creating new dataframe with only the response variables
trees_resp <- trees[, 9:20]
...
# ONF
trees_ONF <- trees[trees$Site == "ONF", ]
trees_explan_ONF <- cbind(trees_ONF[, 1:8], trees_ONF[, 21])
trees_resp_ONF <- trees_ONF[, 9:20]
trees_resp_ONF <- decostand(x = trees_resp_ONF, method = "range")
trees_dist_ONF <- vegdist(x = trees_resp_ONF, method = "euclidean")
# OESF
trees_OESF <- trees[trees$Site == "OESF", ]
trees_explan_OESF <-cbind(trees_OESF[, 1:8], trees_OESF[, 21])
trees_resp_OESF <- trees_OESF[, 9:20]
...
ggbiplot(trees_PCA, alpha = 0, var.axes = FALSE) +
  ggtitle("PCA") +
  geom_point(aes(colour = trees$Species, shape = trees$Site)) +
  ...
  scale_shape_manual(name = "Site",
                     labels = c("OESF", "ONF"),
                     values = c(15, 17))
# looking at site for Comp.2
trees_PCA_Scores <- data.frame(trees_PCA$scores, Site = trees$Site)
adonis2(trees_PCA_Scores$Comp.2 ~ Site,
        data = trees_PCA_Scores,
        method = "euc")
ggplot(data = trees_PCA_Scores, aes(x = Site, y = Comp.2)) +
  geom_boxplot() +
  geom_jitter(aes(colour = Site), width = 0.3, height = 0) +
  theme_bw() +
  ggtitle("All Trees PCA, Comp.2-Site Comparison")
ggbiplot(trees_PCA) +
  geom_point(aes(colour = trees$Species)) +
  labs(colour = "Species") +
  ggtitle("PCA with Species")
...
adonis2(trees_PCA_Scores$Comp.1 ~ Species,
        data = trees_PCA_Scores,
        method = "euc")
# results are significant but only explain about 2% of the variance
...
# more plotting
ordiplot_allTrees <- gg_ordiplot(ord = trees_PCA,
                                 groups = trees$Species,
                                 ellipse = FALSE,
                                 hull = TRUE,
                                 spiders = TRUE)
ordiplot_allTrees$plot + scale_colour_manual(values = c("sienna3", "mediumseagreen")) +
  labs(color = "Species", title = "All Trees")
# trying out same as above but with ONF and OESF sites separated
# first, ONF
trees_PCA_ONF <- princomp(trees_resp_ONF, cor = TRUE)
# summary of PCA
summary_PCA_ONF <- summary(trees_PCA_ONF, loadings = TRUE, cutoff = 0)
summary_PCA_ONF$loadings
# write.table(summary_PCA_ONF$loadings, file = "tables/summary_PCA_ONF.csv")
plot(trees_PCA_ONF,
     main = "PCA; ONF")
# plotting
biplot(trees_PCA_ONF)
ggbiplot(trees_PCA_ONF) +
  ggtitle("PCA; ONF")
ggbiplot(trees_PCA_ONF, alpha = 0, var.axes = FALSE) +
  geom_point(aes(colour = trees_ONF$Species)) +
  labs(colour = "Species") +
  ggtitle("ONF PCA with Species") +
  scale_colour_manual(name = "Species",
                      labels = c("THPL", "TSHE"),
                      values = c("sienna3", "mediumseagreen"))
ggbiplot(trees_PCA_ONF) +
  geom_point(aes(colour = trees_ONF$Species)) +
  labs(colour = "Species") +
  ggtitle("PCA with Species; ONF")
# focusing on just first PC
trees_PCA_ONF_Scores <- data.frame(trees_PCA_ONF$scores, Species = trees_ONF$Species)
adonis2(trees_PCA_ONF_Scores$Comp.1 ~ Species,
        data = trees_PCA_ONF_Scores,
        method = "euc")
# p-value is significant; explains about 21% of variance
ggplot(data = trees_PCA_ONF_Scores, aes(x = Species, y = Comp.1)) +
  geom_boxplot() +
  geom_jitter(aes(colour = Species), width = 0.3, height = 0) +
  theme_bw() +
  ggtitle("PCA Comp.1 Species Comparison; ONF") +
  scale_colour_manual(values = c("sienna3", "mediumseagreen"))
ordiplot_ONF <- gg_ordiplot(ord = trees_PCA_ONF,
                            groups = trees_ONF$Species,
                            ellipse = FALSE,
                            hull = TRUE,
                            spiders = TRUE)
ordiplot_ONF$plot + scale_colour_manual(values = c("sienna3", "mediumseagreen")) +
  labs(color = "Species", title = "ONF")
# next, OESF
trees_PCA_OESF <- princomp(trees_resp_OESF, cor = TRUE)
# summary of PCA
summary_PCA_OESF <- summary(trees_PCA_OESF, loadings = TRUE, cutoff = 0)
summary_PCA_OESF$loadings
# write.table(summary_PCA_OESF$loadings, file = "tables/summary_PCA_OESF.csv")
plot(trees_PCA_OESF,
     main = "PCA; OESF")
# plotting
biplot(trees_PCA_OESF)
ggbiplot(trees_PCA_OESF) +
  ggtitle("PCA; OESF")
ggbiplot(trees_PCA_OESF, alpha = 0, var.axes = FALSE) +
  geom_point(aes(colour = trees_OESF$Species)) +
  labs(colour = "Species") +
  ggtitle("OESF PCA with Species") +
  scale_colour_manual(name = "Species",
                      labels = c("THPL", "TSHE"),
                      values = c("sienna3", "mediumseagreen"))
# focusing on just first PC
trees_PCA_OESF_Scores <- data.frame(trees_PCA_OESF$scores, Species = trees_OESF$Species)
adonis2(trees_PCA_OESF_Scores$Comp.1 ~ Species,
        data = trees_PCA_OESF_Scores,
        method = "euc")
# p-value is significant, but only explains around 12% of the variance
ggplot(data = trees_PCA_OESF_Scores, aes(x = Species, y = Comp.1)) +
  geom_boxplot() +
  geom_jitter(aes(colour = Species), width = 0.3, height = 0) +
  theme_bw() +
  ggtitle("PCA Comp.1 Species Comparison; OESF") +
  scale_colour_manual(values = c("sienna3", "mediumseagreen"))
orggbiplot(trees_PCA_OESF) +
  geom_point(aes(colour = trees_OESF$Species)) +
  labs(colour = "Species") +
  ggtitle("PCA with Species; ONF")
# more plotting
ordiplot_OESF <- gg_ordiplot(ord = trees_PCA_OESF,
                             groups = trees_OESF$Species,
                             ellipse = FALSE,
                             hull = TRUE,
                             spiders = TRUE)
ordiplot_OESF$plot + scale_colour_manual(values = c("sienna3", "mediumseagreen")) +
  labs(color = "Species", title = "OESF")


## overall: Site effect still very much present, but differentiation in ONF slightly worse (makes sense as the shadows 
  # were likely disproportionately impacting TSHEs) and OESF differentiation in OESF slightly better (shadows were impacting 
  # small TSHE and THPL basically equally --> causing confusing 'cause shadowed trees 'look' the same?)
